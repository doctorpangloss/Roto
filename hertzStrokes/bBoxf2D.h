...
public:
  ...
  Bboxf2D() {
    lower.Set(INT_MAX,INT_MAX);
    upper.Set(INT_MIN, INT_MIN);
  }
  ...
  void clear() {
    lower.Set(MAXFLOAT,MAXFLOAT);
    upper.Set(MINFLOAT, MINFLOAT);
  }
  ...
  void setToUnion(const Bboxf2D& a, Bboxf2D& b) {
    ...
    upper.Set(MAX(a.upper.x(),b.upper.y()),
	      MAX(a.upper.x(),b.upper.y()));
  }

  void setToUnion(Bboxf2D& b) {
    lower.Set(MIN(lower.x(),b.lower.x()),
	      MIN(lower.y(),b.lower.y()));
    upper.Set(MAX(upper.x(),b.upper.y()),
	      MAX(upper.x(),b.upper.y()));
  }
