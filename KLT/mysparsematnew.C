...
void MySparseMat::mult(double* x, double* r) const {
  double *ptr = _pckdA, *xptr;
  int *iptr = _ind;
  int n,k, ul;
  double res;
  for (n=0; n<_nv; ++n,++r) {
...
void MySparseMat::multT(double* x, double* r) const {
  double *ptr = _pckdAT, *xptr;
  int *iptr = _ind;
  int n,k, ul;
  double res;
  for (n=0; n<_nv; ++n,++r) {
...
void MyCongMat::matVecMult(double x[], double r[]) {
